...
class User:
    ...
    def __init__(self, id, curk, sk, usrName, FullName):
        self.ID = id
        self.k = curk
        self.dID = sk
        self.name = usrName
    ...
    def insert(self, nameList):
        ...
        for uname in nameList:
            if uname not in node.children:
                ...
                if nk == 0:
                    nID.append(num)
                    ndID = self.KeyGen(pairing, l, [num], uname)
                    nUsr = User(nID, 1, ndID, uname, uname)
    ...
    def createNextSk(self, pairing, l, uname):
        ...
        newUsr = User(newID, self.k + 1, newdID, uname, self.FullName + "-" + uname)
    ...
    def Encrypt(self, pairing, l, message):
        ...
        for i in range(0, self.k):
            h_I = Element(pairing, G1, value = publicParams[i + 4] ** self.ID[i])
...
if __name__ == "__main__":
    ...
    # 遍历这棵树, 查看每个节点的私钥长度与名字
    LenRes, NameRes = rootUsr.BFS()
